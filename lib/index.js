...
exports.getStoreEvents = exports.generateBrandColor = exports.generateProductDescription = exports.analyzeProduct = exports.generateInstagramCaptions = exports.onProductUpdate = exports.sendEmailNotification = void 0;
/**
...
exports.sendEmailNotification = (0, https_1.onCall)({
    ...
    if (!mailTransport) {
        throw new https_1.HttpsError('internal', 'Mail secrets not loaded.');
    }
    const { to, subject, htmlBody } = request.data;
    if (!to || !subject || !htmlBody) {
        logger.error("Invalid arguments:", request.data);
        throw new https_1.HttpsError('invalid-argument', 'The function requires "to", "subject", and "htmlBody" fields.');
    }
    if (!isValidEmail(to)) {
    ...
    if (htmlBody.length > 100000) {
        throw new https_1.HttpsError('invalid-argument', 'HTML body too large (max 100KB).');
    }
    const mailOptions = {
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', 'Failed to send email via external service.', error.toString());
    }
});
// --- Function 2: onProductUpdate ---
exports.onProductUpdate = (0, firestore_1.onDocumentUpdated)({
    ...
    try {
        ...
        if (!storeData || !storeData.backInStockEnabled) {
            logger.warn(`Store ${(storeData === null || storeData === void 0 ? void 0 : storeData.name) || storeId} does not have back-in-stock alerts enabled.`);
            return;
        }
        ...
        const lastAlert = (_a = alertData === null || alertData === void 0 ? void 0 : alertData.timestamp) === null || _a === void 0 ? void 0 : _a.toDate();
...
exports.generateInstagramCaptions = (0, https_1.onCall)({
    secrets: ["GEMINI_KEY"],
}, async (request) => {
    if (!request.auth) {
        logger.error("Authentication failed: User was not authenticated.");
  ...
  `;
    try {
        ...
        throw new https_1.HttpsError("internal", "Failed to generate AI captions. Please try again.");
    }
});
// --- Function 4: analyzeProduct ---
exports.analyzeProduct = (0, https_1.onCall)({ secrets: ["GEMINI_KEY"] }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'You must be logged in.');
    }
    const { product, planId } = request.data;
    if (!product) {
        throw new https_1.HttpsError("invalid-argument", "No product data provided.");
    }
    const issues = [];
    // Description Check
...
exports.generateProductDescription = (0, https_1.onCall)({
    secrets: ["GEMINI_KEY"],
}, async (request) => {
    if (!request.auth) {
        logger.error("Authentication failed: User was not authenticated.");
  ...
  `;
    try {
        ...
        logger.error("AI Generation Error:", error);
        throw new https_1.HttpsError("internal", "Failed to generate AI description. Please try again.");
    }
});
// --- Function 6: generateBrandColor ---
exports.generateBrandColor = (0, https_1.onCall)({
    secrets: ["GEMINI_KEY"],
}, async (request) => {
    if (!request.auth) {
        logger.error("Authentication failed: User was not authenticated.");
  ...
  `;
    try {
        ...
        logger.error("AI Generation Error:", error);
        throw new https_1.HttpsError("internal", "Failed to generate brand color. Please try again.");
    }
});
...
exports.getStoreEvents = (0, https_1.onCall)({}, (request) => {
