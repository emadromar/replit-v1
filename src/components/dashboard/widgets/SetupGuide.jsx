...
export const SetupGuide = ({ store, products, onOpenUpgradeModal }) => {
  ...
  useEffect(() => {
    ...
    const completedCount = newTasks.filter(t => t.isComplete).length;
    if (completedCount > 0 && completedCount < newTasks.length) {
        setIsExpanded(false);
    }
  ...
  const handleDismiss = (e) => {
    e.stopPropagation();
    if (window.confirm("Hide the setup guide forever? You can find these settings in the sidebar later.")) {
    ...
    const Content = () => (
      <div className="flex items-center justify-between w-full">
        <div className="flex items-center min-w-0 gap-3">
          <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center border transition-colors ${
            task.isComplete 
        ...
        {isLocked && !task.isComplete && (
          <span className="flex-shrink-0 text-[10px] font-bold uppercase tracking-wider text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full border border-indigo-100 flex items-center">
    ...
    if (isLocked && !task.isComplete) {
      return (
        <button 
          ...
          className="w-full p-3 bg-white border border-gray-200 rounded-xl hover:border-indigo-300 hover:shadow-sm transition-all text-left"
        >
          <Content />
        </button>
      );
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        <motion.div 
          className="card overflow-hidden p-0 border-0 shadow-sm" 
        ...
        >
          {/* HEADER */}
          <div 
            className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer bg-white hover:bg-gray-50 transition-colors group border border-gray-200 rounded-xl relative z-10"
          ...
          >
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium">{completedTasks}/{tasks.length} steps completed</p>
                </div>
            </div>
          ...
          <AnimatePresence>
            {isExpanded && (
                ...
                >
                    <div className="p-4 sm:p-5 bg-gray-50/50 border-x border-b border-gray-200 rounded-b-xl mt-[-4px] pt-6">
